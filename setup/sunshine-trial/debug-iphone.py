...
props = ['User=luque', 'AmbientCapabilities=CAP_SYS_ADMIN', 'CapabilityBoundingSet=CAP_SYS_ADMIN',
 ...
 'RestrictSUIDSGID=yes', 'RuntimeMaxSec=900', 'TimeoutStopSec=3', 'MemoryMax=1G', 'CPUQuota=200%',
 f'ReadWritePaths={trial}/state', f'StandardOutput=append:{trial}/state/debugger.log', f'StandardError=append:{trial}/state/debugger.log', f'BindReadOnlyPaths={trial}/root/usr/share/sunshine:/usr/share/sunshine']
...
cmd=['pkexec','/usr/bin/systemd-run','--unit=phonepad-sunshine-trial','--collect','--wait']
for prop in props: cmd += ['-p',prop]
cmd += [f'--setenv={k}={v}' for k,v in env.items()]
cmd += ['/usr/bin/gdb', '--batch', '-iex', 'set auto-load off', '-ex', 'set pagination off', '-ex', 'run', '-ex', 'thread apply all bt 12', '--args', str(trial/'root/usr/bin/sunshine'),str(trial/'state/iphone.conf')]
