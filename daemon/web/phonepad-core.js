...
exports.CursorReceiver = exports.CURSOR_POINTS = void 0;
exports.cursorPlacement = cursorPlacement;
exports.CURSOR_POINTS = 28;
...
class CursorReceiver {
    ...
    accept(text) {
        ...
        try {
            ...
            if (m.t !== 'cursor' || m.version !== 1 || !Number.isSafeInteger(m.sequence) || m.sequence <= this.sequence || typeof m.visible !== 'boolean')
                return null;
            if (![m.sourceWidth, m.sourceHeight].every(v => Number.isInteger(v) && v > 0 && v <= 16384))
                return null;
            if (!m.visible) {
                this.sequence = m.sequence;
                return { visible: false, x: 0, y: 0, hx: 0, hy: 0, w: 1, h: 1, sourceWidth: m.sourceWidth, sourceHeight: m.sourceHeight, imageId: '', image: '' };
            }
            if (![m.x, m.y, m.hx, m.hy, m.w, m.h].every(Number.isFinite) || m.w < 1 || m.h < 1 || m.w > 256 || m.h > 256 || m.hx < 0 || m.hy < 0 || m.hx > m.w || m.hy > m.h || Math.abs(m.x) > 32768 || Math.abs(m.y) > 32768 || !/^[0-9a-f]{16}$/.test(m.imageId))
                return null;
            if (m.image !== undefined) {
                if (typeof m.image !== 'string' || m.image.length > 60000 || !/^data:image\/png;base64,iVBORw0KGgo[A-Za-z0-9+/]*={0,2}$/.test(m.image))
                    return null;
                this.image = m.image;
                this.imageId = m.imageId;
            }
            this.sequence = m.sequence;
...
exports.CursorReceiver = CursorReceiver;
/** Transform only the hotspot position. Bitmap dimensions remain in UI points. */
function cursorPlacement(cursor, width, height, scale = 1, panX = 0, panY = 0, points = exports.CURSOR_POINTS) {
    'worklet';
    if (!cursor?.visible || !cursor.image || width <= 0 || height <= 0)
        return { opacity: 0, left: 0, top: 0, width: 1, height: 1 };
...
const buffer_1 = load(2);
const Crypto = __importStar(load(5));
...
const sha2_js_1 = load(7);
const utils_js_1 = load(9);
 ...
 * @module
 */
const _md_js_1 = load(8);
const u64 = __importStar(load(10));
const utils_js_1 = load(9);
/**
 * Round constants:
...
class DirectPointerSequence {
    ...
    frame(points) {
        if (!points.length) {
            ...
            if (this.pressed)
                this.release();
            else if (!this.moved && !this.held && this.start) {
...
const media_capabilities_1 = load(14);
...
}];const cache={};function load(id){if(!cache[id]){const module={exports:{}};cache[id]=module;modules[id](module,module.exports,load);}return cache[id].exports;}window.PhonepadCore=Object.assign({},load(0),load(1),load(6),load(11),load(12),load(13));})();
