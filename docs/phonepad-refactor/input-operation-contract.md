...
Estado: núcleo de staging de texto implementado y probado en `daemon/internal/inputops`. Todavía no está expuesto por HTTP/WebSocket ni conectado al inyector. No anunciar estas capacidades a un cliente hasta completar esa integración. El protocolo `k` anterior sigue activo.
...
Solo `text.literal` tiene núcleo ejecutable en este checkpoint. Las otras familias deben usar la misma identidad, recibos y barreras; no crear protocolos paralelos.
...
## Integración que falta antes de habilitarlo
...
No se habilita soporte de pegado largo en la UI solamente por existir este núcleo. Se mantiene el límite anterior hasta completar el camino entero sin truncar ni ejecutar Enter.
