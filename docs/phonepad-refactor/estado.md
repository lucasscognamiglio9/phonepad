...
`a14291d` registra la fixture de movimiento de libinput. Cuatro geometrías con ocho trazas cada una mostraron un cambio aproximado de 1,43 veces en ganancia por punto al girar con el mapeo actual. El candidato isotrópico mantuvo la ganancia entre orientaciones. El análisis conserva las unidades de libinput, rechaza trazas incompletas y pasó sobre la evidencia guardada. `c5624bf` midió después dos geometrías cuadradas 100x100 mm. La ganancia X difirió aproximadamente 0,03% entre orientaciones y la de Y 1,7%. Falta medir transición sobre la misma instancia y calibrar recorrido/precisión con velocidad lógica idéntica y cursor observable. No cambió el trackpad de producción y P04.7–P04.9 siguen abiertas. Ver `p04-01-input-design.md` y `p04-02-motion-lab.md`.
...
`4639ce0` corrige los estados de flechas, Esc, Tab, Copiar, Pegar, modificadores y Enter cuando hay borrador, envío o revisión pendiente. Unifica el deshabilitado y añade una explicación. Pasaron las 29 pruebas del compositor y TypeScript. Los recibos remotos de acciones siguen pendientes; ver `p01a-05-shortcut-controls.md`.
